...
def show_frames(cap, label):
    ...
    if ret:
        ...
        cv2image = cv2.resize(cv2image, (720, 600))

        img = Image.fromarray(cv2image)

        crop_img = cv2image[60:660, 0:600]
        crop_img = Image.fromarray(crop_img)
        # Convert image to PhotoImage
        imgtk = ImageTk.PhotoImage(image = crop_img)
        label.imgtk = imgtk
...
def capture(win, cap, label):
    ...
    crop_img = Image.fromarray(crop_img)
    # Convert image to PhotoImage
    imgtk = ImageTk.PhotoImage(image=crop_img, master=win)
    label.imgtk = imgtk
...
def request_portrait(win, label):
    ...
    # portrait_img_path = os.path.join("portraits", f"{name}__{email}.png")
    # cv2.imread(portrait_img_path, cv2.IMREAD_GRAYSCALE)
    # cv2image = cv2.cvtColor(portrait_img_path, cv2.COLOR_BGR2RGB)
    # cv2image = cv2.resize(cv2image, (480, 480))
    # imgtk = ImageTk.PhotoImage(image=cv2image, master=win)
    # label.imgtk = imgtk
    # label.config(image=imgtk)
